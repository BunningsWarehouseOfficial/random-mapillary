...
def main():
    ...
    for ii, img_path in enumerate(imgs):
        ...
        if pressedKey == ord('p') or pressedKey == ord('P'):
            shutil.move(img_path, os.path.join(passdir, os.path.basename(img_path)))
            print(f"[{ii + 1}] Marked as PASS: {os.path.basename(img_path)}")
        elif pressedKey == ord('f') or pressedKey == ord('F'):
            shutil.move(img_path, os.path.join(faildir, os.path.basename(img_path)))
            print(f"[{ii + 1}] Marked as FAIL: {os.path.basename(img_path)}")
        elif pressedKey == ord('r') or pressedKey == ord('R'):
            shutil.move(img_path, os.path.join(recheckdir, os.path.basename(img_path)))
            print(f"[{ii + 1}] Marked as RECHECK: {os.path.basename(img_path)}")
        elif pressedKey == ord('o') or pressedKey == ord('O'):
            shutil.move(img_path, os.path.join(otherdir, os.path.basename(img_path)))
